...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${label} in Austin | Local Guide`,
    description: `Discover the best ${label.toLowerCase()} in Austin, Texas. Find local favorites, popular spots, and hidden gems.`,
    keywords: [
      `${label.toLowerCase()} Austin`,
      `best ${label.toLowerCase()} Austin TX`,
      `Austin ${label.toLowerCase()} guide`,
    ],
  }
}
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  return (
    <>
      <SEOPageHero
        title={`${categoryLabel} in Austin`}
        subtitle={`Discover ${places.length} local ${categoryLabel.toLowerCase()} in Austin`}
